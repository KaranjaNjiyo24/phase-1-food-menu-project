document.addEventListener("DOMContentLoaded", () => {
    ...
    const maxItems = 10;  // The number of unique food items to display

    // Function to fetch images until we have 10 unique categories
    function fetchUniqueImages() {
        ...
        fetch(baseUrl)
            ...
            .then(data => {
                ...
                const category = extractCategoryFromUrl(imageUrl);  // Function to extract category from URL

                // Check if the category is already in the set
                if (!uniqueCategories.has(category)) {
                    uniqueCategories.add(category);  // Add to the set if it's unique
                    const dishName = formatDishName(category);  // Format our category to a readable dish name
                    ...
                    menuItems.push({
                        ...
                        likes: 0,
                        dislikes: 0,
                        liked: false,    // Track whether the item is liked
                        disliked: false, // Track whether the item is disliked
                        comments: []
                    });
                }

                // Continue fetching until we have enough unique items
                fetchUniqueImages();
            });
    }
